...
contract ProsperoPrices{
  ...
  function getHelperToken(uint256 i) private pure returns(address token){
    if(i==0){
      return WAVAX;
    }else if (i==1){
      return USDC;
      ...
      if (reserve0!=0 && reserve1!=0){
        ...
        uint256 valueOfLiquidityOfHelper= (
          (
            ...
            /
            (10**(ERC20(thisPath[1]).decimals()))
          )
        );
        if (valueOfLiquidityOfHelper < (MINIMUM_LIQUIDITY_REQUIRED * USD_SCALE)){
        }else{
          try
          IPangolinRouter(PANGOLIN_ROUTER_ADDRESS).getAmountsOut((10**(ERC20(tokenAddress).decimals())), thisPath)
          returns (uint256[] memory amounts) {
            if (amounts[amounts.length - 1]>0){
            uint256 estimatedPriceOfToken = ((amounts[amounts.length - 1] * priceScaled) / (10**(ERC20(thisPath[1]).decimals())));
